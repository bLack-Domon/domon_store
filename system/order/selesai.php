...
$select_invoice = $server->query("SELECT * FROM `invoice`, `iklan`, `kategori` WHERE invoice.id_user='$iduser' AND invoice.tipe_progress='Selesai' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id ORDER BY `invoice`.`idinvoice` DESC");
...
?>
    <div class="box_isi_res_order">
        ...
        while ($invoice_data = mysqli_fetch_assoc($select_invoice)) {
            ...
            $idivrat = $invoice_data['idinvoice'];
        ?>
            <div class="isi_cart" id="isi_cart<?php echo $invoice_data['id']; ?>">
                ...
                <div class="box_detail_isi_cart">
                    ...
                    <?php
                    $select_rating_iv = $server->query("SELECT * FROM `rating` WHERE `id_invoice_rat`='$idivrat' ");
                    $data_rating_iv = mysqli_fetch_assoc($select_rating_iv);
                    if ($data_rating_iv) {
                    ?>
                        <div class="bayar dspn">Beri Nilai</div>
                        <p class="snilai dspb">Sudah Dinilai</p>
                    <?php
                    } else {
                    ?>
                        <div class="bayar dspb" id="bu_snilai<?php echo $invoice_data['idinvoice']; ?>" onclick="show_bp('<?php echo $invoice_data['idinvoice']; ?>')">Beri Nilai</div>
                        <p class="snilai dspn" id="snilai<?php echo $invoice_data['idinvoice']; ?>">Sudah Dinilai</p>
                    <?php
                    }
                    ?>
                </div>
